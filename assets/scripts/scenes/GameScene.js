class GameScene extends Phaser.Scene {
    constructor() {
        super('Game');
    }
    init() {
        this.cursors = this.input.keyboard.createCursorKeys();
        this.score = 0;
    }
    create() {
        this.createBackground();
        console.log(this.sounds)
        if (!this.sounds) {
            this.createSounds();
        }
        this.player = new Player(this);
        this.enemies = new Enemies(this);
        this.createText();
        this.createCompleteEvents();
        this.addOverlap();
    }
    createText() {
        this.scoreText = this.add.text(50, 50, 'Score: '+ this.score, {
            font: '50px RomanSD',
            fill: '#fff'
        })
    }
    createSounds() {
        this.sounds = {
            boom: this.sound.add('boom'),
            theme: this.sound.add('theme'),
        };

        this.sounds.theme.play({
            volume: 0.05,
            loop: true,
        });
    }
    addOverlap() {
        this.physics.add.overlap(this.player.fires, this.enemies, this.onOverlap, undefined, this);
        this.physics.add.overlap(this.enemies.fires, this.player, this.onOverlap, undefined, this);
        this.physics.add.overlap(this.player, this.enemies, this.onOverlap, undefined, this);
    }
    onOverlap(source, target) {
        const enemy = [source, target].find(item => item.texture.key === 'enemy');
        source.setAlive(false);
        target.setAlive(false);
        if (enemy) {
            ++this.score;
            this.scoreText.setText('Score: '+ this.score)
            this.boom = new Boom({scene: this, x: enemy.x, y: enemy.y});
            this.sounds.boom.play({
                volume: 0.5,
            });
        }
    }
    createCompleteEvents() {
         // this.player.once/on/emit  получение у игрового объекта доступа к событиям разовому/многоразовому/выпуску событий
         this.player.once('killed', this.onComplete, this)  // отслеживание события killed
         // this.enemies.once('killed', this.onComplete, this) 
         this.events.on('enemies-killed', this.onComplete, this)
        }
    onComplete() {
        this.scene.start("Start", {
            score: this.score,
            completed: this.player.active,
        });
    }
    update() {
        this.player.move();
        this.bg.tilePositionX += 0.5;
    }
    createBackground() {
        this.bg = this.add.tileSprite(0, 0, config.width, config.height, 'bg').setOrigin(0);
    }
}